...
from django2use.panel.models import Machine

def dynamic_dns_read(request, domain):
    data = {}
    dns_record = Machine.objects.get(domain=domain)
    data['domain'] = dns_record.hostname
    data['ip'] = dns_record.ip4_addr
    data['last_change'] = dns_record.last_time.isoformat()
...
def dynamic_dns_update(request, domain):
    ...
    if 'key' not in request.POST:
        ...
        if not dns_record or request.POST['key'] != dns_record.token:
            ...
            if ip != dns_record.ip4_addr:
                dns_record.ip4_addr = ip
                dns_record.save()
                updated = True
            data['domain'] = dns_record.hostname
            data['ip'] = dns_record.ip4_addr
