...
# ! Never use the timezone now, import django2use.event.utils.now instead always
...
class EventUpdateViewTestCase(ViewRequestFactoryTestMixin, TestCase):
    ...
    def setUp(self):
        self.event = mixer.blend('django2use.event.Event')
...
class OccurrenceViewTestCaseMixin(object):
    ...
    def setUp(self):
        self.rule = mixer.blend('django2use.event.Rule', name='daily')
        self.event = mixer.blend(
            'django2use.event.Event', created_by=mixer.blend('auth.User'),
...
class OccurrenceDeleteViewTestCase(
    ...
    def test_deletion(self):
        ...
        new_rule = mixer.blend('django2use.event.Rule', name='weekly',
                               frequency='WEEKLY')
        new_event = mixer.blend(
            'django2use.event.Event',
            rule=new_rule,
...
class UpcomingEventsAjaxViewTestCase(ViewRequestFactoryTestMixin, TestCase):
    ...
    def test_view_with_category(self):
        cat = mixer.blend('django2use.event.EventCategory')
